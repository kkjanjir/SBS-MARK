...
export default function MarksheetApp() {
  ...
  // ⚡ Bolt Optimization: Wrap helper functions in useCallback to avoid stale closures and ensure stable references in dependencies
  ...
  // ⚡ Bolt Optimization: Memoize the list of students in the active class to prevent recalculation on every render
  const classFilteredStudents = useMemo(() => dbStudents.filter(s => s.class_name === activeClass), [dbStudents, activeClass]);

  // ⚡ Bolt Optimization: Memoize the heavy rendering of the bulk marksheet container to prevent O(N) re-renders
  const printBulkContent = useMemo(() => {
