...
const getClassSubjects = (clsName: string, subjectConfig: Record<string, string[]>, mObj?: MarksState) =>
...
export default function MarksheetApp() {
  ...
  const currentSubjectsList = getClassSubjects(activeClass, subjectConfig, marks);
    ...
    return classFilteredStudents.map((s, index) => {
      ...
      return (
        <div key={s.id} className="marksheet-page" style={{ pageBreakAfter: index === classFilteredStudents.length - 1 ? 'auto' : 'always' }}>
          <MarksheetTemplate theme={THEMES[activeTheme]} student={s.student_data || {}} marks={safeMarks} subjectsList={getClassSubjects(s.class_name, subjectConfig, safeMarks)} grandTotal={calcs.grandTotal} percentage={calcs.percentage} finalGrade={calcs.finalGrade} extra={s.extra_data || {}} coScholastic={s.extra_data?.coScholastic || {sports:'A',art:'A',music:'A',discipline:'A'}} photo={s.student_data?.photo} rank={getClassRank(calcs.grandTotal, s.class_name)} activeClass={s.class_name} showTableWatermark={showTableWatermark} />
        </div>
      )
    });
  }, [classFilteredStudents, subjectConfig, activeTheme, showTableWatermark, getClassRank]);
