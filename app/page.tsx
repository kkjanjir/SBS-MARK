...
export default function MarksheetApp() {
  ...
  const getDynamicSubjects = useCallback((mObj: MarksState | null | undefined, fallback: string[]) => {
  ...
  const getClassSubjects = useCallback((clsName: string, mObj?: MarksState) => getDynamicSubjects(mObj, subjectConfig[clsName] || DEFAULT_SUBJECTS), [getDynamicSubjects, subjectConfig]);
  ...
  const getGrade = useCallback((marksObtained: number | string, maxMarks: number) => {
  ...
  // 🚀 OPTIMIZATION: Memoize filtered students to avoid O(N) filtering on every render
  const classFilteredStudents = useMemo(() => {
    return dbStudents.filter(s => s.class_name === activeClass);
  }, [dbStudents, activeClass]);

  // 🚀 OPTIMIZATION: Heavily memoize the hidden bulk print container
  // This prevents O(N) React render cycles of complex MarksheetTemplates during unrelated typing/state changes.
  ...
  }, [classFilteredStudents, activeTheme, showTableWatermark, getClassSubjects, getCalculations, getClassRank]);
