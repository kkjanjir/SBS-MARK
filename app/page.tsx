...
export default function MarksheetApp() {
  ...
  const subjectsCache = useMemo(() => new Map(), [subjectConfig]);
  const getClassSubjects = useCallback((clsName: string, mObj?: MarksState) => {
    const fallback = subjectConfig[clsName] || DEFAULT_SUBJECTS;
    const markKeys = Object.keys(mObj || {}).filter(Boolean).sort();
    const key = clsName + '|' + markKeys.join(',');

    if (subjectsCache.has(key)) return subjectsCache.get(key);

    const result = getDynamicSubjects(mObj, fallback);
    subjectsCache.set(key, result);
    return result;
  }, [subjectConfig, subjectsCache]);
  ...
  // Optimization: Memoize the filtered students to prevent recalculation on every render
  const classFilteredStudents = useMemo(() => {
    return dbStudents.filter(s => s.class_name === activeClass);
  }, [dbStudents, activeClass]);

  // Memoize the bulk print array to prevent O(N) re-renders
  const bulkPrintMarkSheets = useMemo(() => {
