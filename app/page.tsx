...
const getGrade = (marksObtained: number | string, maxMarks: number) => {
  if (marksObtained === '' || marksObtained === undefined) return '';
...
// Cache dynamic subjects to preserve referential equality for React.memo
...
const getDynamicSubjects = (mObj: MarksState | null | undefined, fallback: string[]) => {
  ...
  const cacheKey = markKeys.sort().join(',') + '|' + fallback.join(',');
  if (dynamicSubjectsCache.has(cacheKey)) {
    return dynamicSubjectsCache.get(cacheKey)!;
  }
...
export default function MarksheetApp() {
  ...
  // We use useMemo to cache the massive bulk printing map operation.
  // This prevents O(N) MarksheetTemplate re-renders and freezing on every simple state change.
