...
import { MarksState, getGrade, getDynamicSubjects, getCalculations } from './utils';
...
export default function MarksheetApp() {
  ...
  const saveToCloud = async (addNext: boolean = false) => {
    ...
    let myTotal = getCalculations(marks, getClassSubjects(activeClass, marks)).grandTotal;
  ...
  const { grandTotal, percentage, finalGrade } = getCalculations(marks, getClassSubjects(activeClass, marks));
      ...
      const newLocalRecords: BackupRecord[] = rows.map((cols, rowIndex) => {
        ...
        return {
          ...
          extra_data: {
            ...
            total: getCalculations(marksData, getClassSubjects(recClass, marksData)).grandTotal
          },
          source: 'local',
          created_at: new Date().toISOString()
        };
    ...
    <>
      ...
      <div id="print-bulk-container" className="print-area">
        {classFilteredStudents.map((s, index) => {
          ...
          const calcs = getCalculations(safeMarks, getClassSubjects(s.class_name, safeMarks));
          return (
