...
// ⚡ Bolt Optimization: Cache the dynamic subjects result to prevent recreating arrays
// which would defeat React.memo shallow comparison on MarksheetTemplate
...
const getDynamicSubjects = (mObj: MarksState | null | undefined, fallback: string[]) => {
    ...
    // Create a deterministic cache key based on the fallback string and the mark keys
    const cacheKey = fallback.join(',') + '|' + markKeys.join(',');
    if (subjectsCache.has(cacheKey)) {
        return subjectsCache.get(cacheKey)!;
    }

    const newSubjects = [...fallback.filter(sub => markKeys.includes(sub)), ...markKeys.filter(sub => !fallback.includes(sub))];
    subjectsCache.set(cacheKey, newSubjects);
    return newSubjects;
};
...
// ⚡ Bolt Optimization: Using static default objects outside the component
// prevents recreation on every render, satisfying React.memo\'s shallow comparison
const defaultStudent = {};
const defaultMarks = {};
const defaultExtra = {};
const defaultCoScholastic = { sports: 'A', art: 'A', music: 'A', discipline: 'A' };

// ⚡ Bolt Optimization: Memoize the template to prevent O(N) re-renders during bulk printing
