...
const getDynamicSubjectsCache = new Map<string, string[]>();
const getDynamicSubjects = (mObj: MarksState | null | undefined, fallback: string[]) => {
  const markKeys = Object.keys(mObj || {}).filter(Boolean).sort();
  if (markKeys.length === 0) return fallback;

  const cacheKey = markKeys.join(',') + '|' + fallback.join(',');
  if (getDynamicSubjectsCache.has(cacheKey)) {
    return getDynamicSubjectsCache.get(cacheKey)!;
  }

  const result = [...fallback.filter(sub => markKeys.includes(sub)), ...markKeys.filter(sub => !fallback.includes(sub))];
  getDynamicSubjectsCache.set(cacheKey, result);
  return result;
};
...
export default function MarksheetApp() {
  ...
  const getCalculations = useCallback((mObj: MarksState | undefined | null, clsName: string) => {
    const safeMObj = mObj || {};
    const subs = getDynamicSubjects(safeMObj, subjectConfig[clsName] || DEFAULT_SUBJECTS);
    let gTotal = 0;
  ...
  const bulkPrintContent = useMemo(() => {
    return classFilteredStudents.map((s, index) => {
      const safeMarks = s.marks_data || defaultMarks;
      ...
      return (
        <div key={s.id} className="marksheet-page" style={{ pageBreakAfter: index === classFilteredStudents.length - 1 ? 'auto' : 'always' }}>
          <MemoizedMarksheetTemplate
            ...
            extra={s.extra_data || defaultExtra}
  ...
  }, [classFilteredStudents, activeTheme, showTableWatermark, subjectConfig, dbStudents, getCalculations, getClassSubjects, getClassRank]);
    ...
    <>
            ...
            <div className="flex-1 flex flex-col lg:flex-row w-full">
              ...
              <div className="w-full lg:w-[55%] bg-gray-800 lg:p-6 flex justify-center overflow-auto relative">
                ...
                <div className="lg:origin-top lg:scale-[0.70] xl:scale-[0.80] transition-transform">
                  <MemoizedMarksheetTemplate templateId="marksheet-preview" theme={THEMES[activeTheme]} student={student || defaultStudent} marks={marks || defaultMarks} subjectsList={currentSubjectsList} grandTotal={grandTotal} percentage={percentage} finalGrade={finalGrade} extra={extraDetails || defaultExtra} coScholastic={coScholastic || defaultCoScholastic} photo={studentPhoto} rank={getClassRank(grandTotal, activeClass)} activeClass={activeClass} showTableWatermark={showTableWatermark} />
                </div>
              </div>
            </div>
          </div>
      ...
      <div id="print-single-container" className="print-area">
        <div className="marksheet-page">
          <MemoizedMarksheetTemplate theme={THEMES[activeTheme]} student={student || defaultStudent} marks={marks || defaultMarks} subjectsList={currentSubjectsList} grandTotal={grandTotal} percentage={percentage} finalGrade={finalGrade} extra={extraDetails || defaultExtra} coScholastic={coScholastic || defaultCoScholastic} photo={studentPhoto} rank={getClassRank(grandTotal, activeClass)} activeClass={activeClass} showTableWatermark={showTableWatermark} />
        </div>
      </div>
...
const MemoizedMarksheetTemplate = memo(MarksheetTemplate);
