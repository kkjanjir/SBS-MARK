...
// Utility functions moved outside component for React.memo optimization
const getGradeGlobal = (marksObtained: number | string, maxMarks: number) => {
...
const getDynamicSubjectsGlobal = (mObj: MarksState | null | undefined, fallback: string[]) => {
...
export default function MarksheetApp() {
  ...
  const getClassSubjects = React.useCallback((clsName: string, mObj?: MarksState) => getDynamicSubjectsGlobal(mObj, subjectConfig[clsName] || DEFAULT_SUBJECTS), [subjectConfig]);
  const currentSubjectsList = useMemo(() => getClassSubjects(activeClass, marks), [getClassSubjects, activeClass, marks]);
  ...
  const getCalculations = React.useCallback((mObj: MarksState | undefined | null, clsName: string) => {
    const safeMObj = mObj || {};
    ...
    const fGrade = gTotal > 0 ? getGradeGlobal(gTotal, mMax) : "";
  ...
  const getClassRank = React.useCallback((myTotal: number, clsName: string) => {
    if (myTotal === 0) return "";
    ...
    <>
      ...
      <div id="print-bulk-container" className="print-area">
        {classFilteredStudents.map((s, index) => {
          return <BulkMarksheetRenderer
            key={s.id}
            studentRecord={s}
            isLast={index === classFilteredStudents.length - 1}
...
const BulkMarksheetRenderer = React.memo(function BulkMarksheetRenderer({ studentRecord: s, isLast, activeTheme, getClassSubjects, getCalculations, getClassRank, showTableWatermark }: any) {
  const safeMarks = s.marks_data || DEFAULT_MARKS;
  const calcs = getCalculations(safeMarks, s.class_name);
  const subjectsList = React.useMemo(() => getClassSubjects(s.class_name, safeMarks), [getClassSubjects, s.class_name, safeMarks]);
  const student = s.student_data || DEFAULT_STUDENT;
  const extra = s.extra_data || DEFAULT_EXTRA;
  const coScholastic = s.extra_data?.coScholastic || DEFAULT_COSCHOLASTIC;
  const rank = getClassRank(calcs.grandTotal, s.class_name);

  return (
    <div className="marksheet-page" style={{ pageBreakAfter: isLast ? 'auto' : 'always' }}>
      <MarksheetTemplate
                ...
                {resolvedSubjects.map((sub: string, i: number) => {
                  ...
                  return (
                    <tr key={i} className={`border-b-[2px] ${t.border}`}>
                      ...
                      <td className={`p-1.5 ${t.text}`}>{tot > 0 ? getGradeGlobal(tot, 200) : ''}</td>
                    </tr>
                  )
                })}
              </tbody>
