...
export default function MarksheetApp() {
  ...
  // ⚡ Bolt Optimization: Memoize the filtered students array to prevent O(N) filtering on every render.
  const classFilteredStudents = useMemo(() => dbStudents.filter(s => s.class_name === activeClass), [dbStudents, activeClass]);

  // ⚡ Bolt Optimization: Memoize the heavy bulk print rendering block.
  // This prevents executing O(N) render cycles for elements hidden via display: none
  // when unrelated states (like search inputs or view toggles) change.
  // Impact: Severely reduces application-wide input lag and re-renders by caching the result.
