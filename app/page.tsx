...
export default function MarksheetApp() {
  ...
  // PERFORMANCE: Memoize filtered students to avoid O(N) filtering on unrelated state updates.
  const classFilteredStudents = useMemo(() =>
    dbStudents.filter(s => s.class_name === activeClass),
    [dbStudents, activeClass]
  );

  // PERFORMANCE: Heavy bulk printing section. Elements are hidden with CSS display:none
  // but would normally still run full React render cycles. Memoizing the mapped components
  // here prevents massive application-wide input lag by halting O(N) re-renders when
  // unrelated states change (e.g., typing).
