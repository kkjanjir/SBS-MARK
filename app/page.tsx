...
export default function MarksheetApp() {
  ...
  const bulkPrintElements = useMemo(() => {
    return classFilteredStudents.map((s, index) => {
      ...
      const subjectsList = getClassSubjects(s.class_name, safeMarks);
      ...
      return (
        <div key={s.id} className="marksheet-page" style={{ pageBreakAfter: index === classFilteredStudents.length - 1 ? 'auto' : 'always' }}>
          <MarksheetTemplate theme={THEMES[activeTheme]} student={s.student_data || {}} marks={safeMarks} subjectsList={subjectsList} grandTotal={calcs.grandTotal} percentage={calcs.percentage} finalGrade={calcs.finalGrade} extra={s.extra_data || {}} coScholastic={s.extra_data?.coScholastic || {sports:'A',art:'A',music:'A',discipline:'A'}} photo={s.student_data?.photo} rank={rank} activeClass={s.class_name} showTableWatermark={showTableWatermark} />
        </div>
      );
    });
