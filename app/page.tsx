...
export default function MarksheetApp() {
  ...
  // ⚡ OPTIMIZATION: Memoize base array to prevent invalidating downstream useMemo caches on every render
  const classFilteredStudents = useMemo(() => dbStudents.filter(s => s.class_name === activeClass), [dbStudents, activeClass]);

  // ⚡ OPTIMIZATION: Memoized the bulk print container.
  // This container renders O(N) marksheets and is hidden most of the time via CSS.
  // Memoizing it prevents severe application-wide input lag when unrelated states change.
  // Expected Impact: Eliminates O(N) re-renders of MarksheetTemplate components during regular UI interactions.
