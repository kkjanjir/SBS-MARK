...
export default function MarksheetApp() {
  ...
  // ⚡ Bolt: Memoized with useCallback to maintain referential equality and prevent re-rendering dependent components.
  ...
  // ⚡ Bolt: Cached filtered array with useMemo to avoid O(N) recalculations on unrelated state changes.
  const classFilteredStudents = useMemo(() => dbStudents.filter(s => s.class_name === activeClass), [dbStudents, activeClass]);

  // ⚡ Bolt: Memoized large virtual DOM block mapping with useMemo to prevent severe input lag during unrelated state changes.
