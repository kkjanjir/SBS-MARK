...
export default function MarksheetApp() {
  ...
  const getClassSubjects = React.useCallback((clsName: string, mObj?: MarksState) => getDynamicSubjects(mObj, subjectConfig[clsName] || DEFAULT_SUBJECTS), [subjectConfig]);
  const currentSubjectsList = React.useMemo(() => getClassSubjects(activeClass, marks), [getClassSubjects, activeClass, marks]);
  ...
  const classFilteredStudents = React.useMemo(() => dbStudents.filter(s => s.class_name === activeClass), [dbStudents, activeClass]);


  const bulkPrintContent = React.useMemo(() => {
      ...
      return (
        <div key={s.id} className="marksheet-page" style={{ pageBreakAfter: index === classFilteredStudents.length - 1 ? 'auto' : 'always' }}>
          <MarksheetTemplate
            ...
            student={s.student_data || DEFAULT_STUDENT}
            marks={safeMarks}
            ...
            extra={s.extra_data || DEFAULT_EXTRA}
            coScholastic={s.extra_data?.coScholastic || DEFAULT_CO_SCHOLASTIC}
