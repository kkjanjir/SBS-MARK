...
// ⚡ Bolt Performance Optimization: Extracted pure helper functions (getDynamicSubjects, getGrade)
// outside component scope to prevent unnecessary recreation and invalidating downstream memoization.
// Impact: Reduces garbage collection and improves render speeds across the entire app.
...
export default function MarksheetApp() {
  ...
  // ⚡ Bolt Performance Optimization: Wrapped complex derived helpers in useCallback.
// Impact: Prevents stale closures while keeping reference equality across renders.
  ...
  // ⚡ Bolt Performance Optimization: Memoized derived state for bulk printing.
  const classFilteredStudents = useMemo(() => dbStudents.filter(s => s.class_name === activeClass), [dbStudents, activeClass]);

  // ⚡ Bolt Performance Optimization: Heavily memoized the bulk mapping operation.
  // Impact: Prevents O(N) re-renders of hidden MarksheetTemplate components when parent state changes.
  // Measurement: Significant reduction in input lag (e.g. typing access PIN) for large datasets.
  ...
  }, [classFilteredStudents, THEMES, activeTheme, getCalculations, getClassSubjects, getClassRank, showTableWatermark]);
...
// ⚡ Bolt Performance Optimization: Wrapped complex template component in React.memo
// Impact: Bypasses expensive DOM diffing for off-screen rendering when props remain strictly equal.
